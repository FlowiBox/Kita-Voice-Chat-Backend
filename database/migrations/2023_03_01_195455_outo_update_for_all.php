...
class OutoUpdateForAll extends Migration
{
    ...
    public function up()
    {
        ...
        DB::unprepared(
            'DROP TRIGGER IF EXISTS `update_agrncy_target`; CREATE TRIGGER `update_agrncy_target` AFTER UPDATE ON `user_target` FOR EACH ROW UPDATE agencies SET target_usd = ( SELECT SUM(target_usd * target_agency_share / 100) FROM user_target WHERE agency_id = NEW.agency_id AND add_month = MONTH(CURDATE()) AND add_year = YEAR(CURDATE()) ) WHERE id = NEW.agency_id;'
        );
        DB::unprepared(
